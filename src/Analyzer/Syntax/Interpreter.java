...
public class Interpreter {
    ...
    /**
     * Genera una tabla hash con clave: n√∫mero de linea y valor: posici√≥n del
     * token.
     */
    ...
    public void interpretar(List<Token> listaTks) {
        ...
        do {
            if(nLlamadasRecursivas == 0) {
                throw new InterpreterException("N˙mero de llamadas recursivas demasiado grande.");
            }
            nLinea++;
            try {
                leerSiguienteToken();
                ...
                throw new InterpreterException("LÌnea3 " + nLinea + ": " + e.getMessage() + "\n" + salida + "\n\n" + apo);
            }
            if (!tokenActual.lexemasIguales("TK_FIN_SENT") && !tokenActual.lexemasIguales("TK_SALIDA")) {
                throw new NewLineException("Encontrado '" + this.devolverErrorMatch(tokenActual.getLexema().substring(3)) + "', esperaba 'salto de lÌnea'.");
            }
        }
    ...
    double T_PRIMA(double valor) {
        ...
        if (esDivision()) {
            ...
            double valor2 = F();
            if (valor2 == 0) {
                throw new InterpreterException("Divisi√≥n entre 0.");
            }
            valor /= valor2;
            if (esMultiplicacionDivision()) {
                valor = T_PRIMA(valor);
    ...
    double F() {
        if (esParentesisAbierto()) {
            ...
            try {
                if (tokenActual.getContenido() instanceof Integer) {
                    valor = Integer.parseInt(tokenActual.getContenido()+"");
                }
                else {  //Suponemos que es double entonces
                    valor = Double.parseDouble(tokenActual.getContenido()+"");
                }
            }
            catch (Exception e) {
                throw new InterpreterException("No es n√∫mero entero o double");
            }
        ...
        throw new SyntaxException("Esperaba par√©ntesis abierto, identificador o n√∫mero.");
    }
    ...
    //<editor-fold defaultstate="collapsed" desc="Comparaciones">
        ...
        //<editor-fold defaultstate="collapsed" desc="Aritm√©tica">
        boolean esMasMenos() {
    ...
    void saltar(int linea) {
        ...
        if (existeLinea(linea)) {
            ...
            if (nLinea == linea) {
                throw new InterpreterException("Referencia a la misma lÌnea.");
            }
            ...
            if (!encontrado) {
                throw new InterpreterException("LÌnea no encontrada.1");
            }
        }
        Set<Entry<Integer, Integer>> st = this.mapaLineas.entrySet();
        String aa = "";
        int i = 0;
        for(Entry<?, ?> ee: st) {
        	aa += i + ") " + ee + "\n";
        }
        throw new InterpreterException("LÌnea no encontrada. " + aa);
    }
